...
void StatisticsManager::recordDroneSpawned(int droneId, DroneType type, QPointF position)
{
    ...
    StatisticsEvent event(EventType::DroneSpawned, droneId, position, 0.0, QString("Type: %1").arg(typeStr));
...
{
    QString typeStr = "Standard"; // 统一类型
    StatisticsEvent event(EventType::DroneDestroyed, droneId, position, threatValue, 
                         QString("Type: %1, Threat: %2").arg(typeStr).arg(threatValue));
    m_events.append(event);
...
{
    QString typeStr = "Standard"; // 统一类型
    StatisticsEvent event(EventType::DroneEscaped, droneId, position, 0.0, 
...
{
    StatisticsEvent event(EventType::StrikeExecuted, -1, position, radius, 
                         QString("Destroyed: %1, Radius: %2").arg(dronesDestroyed).arg(radius));
    m_events.append(event);
...
{
    StatisticsEvent event(EventType::InterceptExecuted, droneId, interceptPoint, threatValue, 
                         QString("Target ID: %1, Threat: %2").arg(droneId).arg(threatValue));
    m_events.append(event);
...
{
    StatisticsEvent event(EventType::HighThreatDetected, droneId, QPointF(), threatLevel, 
                         QString("High threat level: %1").arg(threatLevel));
    m_events.append(event);
...
bool StatisticsManager::exportToCsv(const QString& filename) const
{
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        // CSV 头
        stream << "Timestamp,EventType,DroneID,PositionX,PositionY,Value,Details\n";
